...
if (-not (Test-Path -Path $WorkingDir)) {
    ...
    Get-ChildItem -Path $directoryPath -Recurse | Remove-Item -Recurse -Force
}
...
docker build -t $containerRegistry/simple-web-api:$BuildId $SrcDirectory
...
# Replace-InFiles -Path $k8sDir -Token BUILD_ID -With [string]$BuildId
